...
2. **Increase depth.** If surface-level work is done, go deeper — edge cases, error paths (see the project's established 'name: ' + errorMessage(err) prefix convention from adapters/types or cli/config error handling), integration points.
