...
**The migration is one-way.** Older pace versions fail against a migrated database with SQLite error `ON CONFLICT clause does not match`. To downgrade, delete `data/pace.db` (or restore a backup). See README, "Database schema migration (after v0.6.5)".
