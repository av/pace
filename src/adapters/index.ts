...
export async function discoverAdapters(): Promise<Map<string, Adapter>> {
  ...
  try {
    ...
    console.warn(`failed to read adapters dir ${dir}: ${errorMessage(err)}`);
    return adapters;
  }
  ...
  for (const entry of files) {
    ...
    if (typeof entry === "string") {
      ...
      console.warn("bad readdir entry: skipped non-string/Dirent entry");
      continue;
    }
    ...
    try {
      ...
      console.warn(`import error: failed to load adapter ${file}: ${errorMessage(err)}`);
    }
  }
