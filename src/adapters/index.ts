...
export async function discoverAdapters(): Promise<Map<string, Adapter>> {
  ...
  for (const file of files) {
    ...
    try {
      ...
      if (adapter && typeof adapter === "object" && adapter !== null && typeof adapter.name === "string" && adapter.name === adapter.name.trim() && adapter.name.trim() && typeof adapter.fetch === "function") {
