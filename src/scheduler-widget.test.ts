...
describe("scheduler-widget interaction", () => {
  ...
  // -- collectPanels skips widgets --

  test("collectPanels returns empty for image widget", () => {
    const node: LayoutNodeConfig = { image: "https://example.com/img.png" };
    expect(collectPanels(node)).toEqual([]);
  });

  test("collectPanels returns empty for text widget", () => {
    const node: LayoutNodeConfig = { text: "hello world" };
    expect(collectPanels(node)).toEqual([]);
  });

  test("collectPanels returns empty for iframe widget", () => {
    const node: LayoutNodeConfig = { iframe: "https://example.com/embed" };
    expect(collectPanels(node)).toEqual([]);
  });

  test("collectPanels returns only panels from mixed layout with widgets", () => {
    const layout: LayoutNodeConfig = {
      direction: "row",
      children: [
        { image: "https://example.com/img.png" },
        { panel: "news", source: "rss", id: "p1" },
        { text: "markdown notes", format: "markdown" },
        { panel: "counter", source: "stats", id: "p2", display: "counter" },
        { iframe: "https://grafana.example.com/d/1" },
      ],
    };
    const panels = collectPanels(layout);
    expect(panels).toHaveLength(2);
    expect(panels[0].panel).toBe("news");
    expect(panels[1].panel).toBe("counter");
  });

  // -- buildLayoutRuntimeMaps with widgets --

  test("buildLayoutRuntimeMaps ignores widgets, maps only panels", () => {
    const layout: LayoutNodeConfig = {
      direction: "column",
      children: [
        { image: "https://example.com/logo.png" },
        { text: "Welcome dashboard" },
        { panel: "feed", source: "rss", id: "feed-panel" },
      ],
    };
    const maps = buildLayoutRuntimeMaps(layout, ["rss"]);
    // Only the panel should appear in the maps
    expect(maps.dashboardPanels).toHaveLength(1);
    expect(maps.dashboardPanels[0].pid).toBe("feed-panel");
    expect(maps.sourceToPanels.get("rss")).toEqual(["feed-panel"]);
  });

  test("buildLayoutRuntimeMaps with widget-only layout produces empty maps", () => {
    const layout: LayoutNodeConfig = {
      direction: "row",
      children: [
    ...
    expect(maps.dashboardPanels).toHaveLength(0);
    expect(maps.sourceToPanels.size).toBe(0);
    expect(maps.sourceToReadKey.size).toBe(0);
  });
